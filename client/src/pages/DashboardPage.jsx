...
export default function DashboardPage() {
  const { user } = useAuth();
  const { trees, stats, reminders, loading } = useTrees();
        ...
        <div className="flex items-start justify-between gap-4 relative z-10">
          <div className="space-y-1.5 max-w-[230px]">
            ...
            <p className="font-body-sm text-body-sm text-[#D8DFC8]">
              Record a new specimen with GPS coordinates, species ID, and baseline morphometrics.
            </p>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="rounded-lg bg-[#30371A] p-3.5 shadow-sm border border-[#525E31] space-y-1">
          <div className="flex items-center justify-between">
            ...
            <span className={`w-2 h-2 rounded-full ${healthyPct >= 80 ? 'bg-[#A4B566] shadow-[0_0_8px_#A4B566]' : healthyPct >= 50 ? 'bg-[#D99B26]' : 'bg-[#E57373]'}`}></span>
          </div>
          <div className="font-headline-md text-headline-md text-[#F0F3E8] font-bold">
            {loading ? '—' : `${healthyPct}%`}
          </div>
          <div className={`inline-flex px-2 py-0.5 rounded border font-label-sm text-label-sm font-semibold ${
            healthyPct >= 80 ? 'bg-[#3A4320] border-[#5D6A37] text-[#D2DCB4]'
            : healthyPct >= 50 ? 'bg-[#3A331A] border-[#D99B26]/60 text-[#F5C26B]'
            : 'bg-[#431B1B] border-[#E57373]/60 text-[#FFCDD2]'
          }`}>
            {healthyPct >= 80 ? 'Optimal' : healthyPct >= 50 ? 'Fair' : 'Needs Review'}
          </div>
        </div>
        ...
        {!loading && trees.length === 0 && (
          <div className="rounded-lg bg-[#30371A] p-8 border border-[#525E31] border-dashed flex flex-col items-center justify-center gap-3 text-center">
            ...
            <div>
              ...
              <p className="font-body-sm text-body-sm text-[#AAB596] max-w-[240px]">
                Start your botanical fieldwork by registering your first seedling specimen.
              </p>
            </div>
            <button
              onClick={() => navigate('/register-tree')}
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="relative w-12 h-12 rounded-lg bg-[#1D230E] overflow-hidden shrink-0 ring-1 ring-[#525E31] flex items-center justify-center">
                      {photoUrl ? (
                    ...
                    <div className="flex flex-col min-w-0">
                      <div className="flex items-center gap-1.5 truncate">
                        ...
                        <span className="text-[#C2CE9F] font-body-sm text-body-sm truncate">
                          • {tree.nickname || tree.species?.split(' ')[0] || 'Unknown'}
                        </span>
                      </div>
